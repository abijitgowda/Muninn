...
def run_init(install_launchd: bool, console: Console) -> None:
    ...
    if not ollama_ok:
        console.print(f"  Install Ollama: [cyan]brew install ollama && ollama serve[/cyan]")
        console.print(f"  Pull model: [cyan]ollama pull {ingest_model}[/cyan]")
    console.print(f"  First ingest: [cyan]muninn ingest --all --limit 5 -v[/cyan]")
...
def _scaffold_vault(vault_path: Path, console: Console) -> None:
    if DEMO_VAULT.exists():
        ...
        console.print(f"  [green]created vault from demo template[/green]")
    else:
        vault_path.mkdir(parents=True)
        for sub in ("Wiki/Concepts", "Wiki/Entities/People", "Wiki/Entities/Orgs",
                     "Wiki/Topics", "Wiki/Projects",
                     "Wiki/Sources", "Raw/Sources", "Journal", "Inbox"):
            (vault_path / sub).mkdir(parents=True, exist_ok=True)
        console.print(f"  [green]created vault skeleton[/green]")
...
def _install_copilot_plugin(plugin_dir: Path, console: Console) -> None:
    ...
    try:
        ...
        console.print(f"  Install manually: Obsidian → Settings → Community plugins → search \"Copilot\"")
...
def _write_wiki_yaml(vault_name: str, ingest_model: str, query_model: str,
                     sources: list[dict], console: Console) -> None:
    import yaml
    config = {
        "settings": {
    ...
    if path.exists():
        if not typer.confirm("wiki.yaml exists (possibly from setup-obsidian). Overwrite with your new config?", default=True):
            console.print(f"  keeping existing wiki.yaml")
            return
    path.write_text("# Muninn — config (generated by `muninn init`)\n\n" +
                    yaml.safe_dump(config, sort_keys=False, default_flow_style=False))
    console.print(f"  [green]wrote wiki.yaml[/green]")
...
def _write_env(console: Console) -> None:
    ...
    if path.exists():
        if not typer.confirm(".env exists. Overwrite?", default=False):
            console.print(f"  keeping existing .env")
            return
    if EXAMPLE_ENV.exists():
        shutil.copy(EXAMPLE_ENV, path)
        console.print(f"  [green]wrote .env[/green]")


# ---- Source builders ----

def _browser_source(name: str, browser: str, path: Path) -> dict:
    return {"name": name, "type": "browser_history", "enabled": True,
            "url": f"file://{path}", "tool": "muninn.sources.browser_history:BrowserHistorySource",
            "options": {"browser": browser, "min_visit_count": 1, "max_items": 500, "fetch_body": True,
                        "exclude_domains": ["google.com", "github.com", "duckduckgo.com", "bing.com",
                                            "localhost", "127.0.0.1"]}}

def _firefox_source(path: Path) -> dict:
    return {"name": "firefox-history", "type": "browser_history", "enabled": True,
            "url": f"file://{path}", "tool": "muninn.sources.firefox_history:FirefoxHistorySource",
            "options": {"min_visit_count": 1, "max_items": 500, "fetch_body": True,
                        "exclude_domains": ["google.com", "github.com", "duckduckgo.com", "bing.com",
                                            "localhost", "127.0.0.1"]}}

def _safari_source(path: Path) -> dict:
    return {"name": "safari-history", "type": "browser_history", "enabled": True,
            "url": f"file://{path}", "tool": "muninn.sources.safari_history:SafariHistorySource",
            "options": {"min_visit_count": 1, "max_items": 500, "fetch_body": True,
                        "exclude_domains": ["google.com", "github.com", "duckduckgo.com", "bing.com",
                                            "localhost", "127.0.0.1"]}}

def _x_source(user_id: str, handle: str) -> dict:
    return {"name": "x-twitter", "type": "x", "enabled": True,
            "url": f"https://api.twitter.com/2/users/{user_id}/tweets",
            "secret_env": "X_BEARER_TOKEN", "tool": "muninn.sources.x:XSource",
            "options": {"user_id": user_id, "handle": handle,
                        "interaction_types": ["timeline", "liked", "replies"],
                        "max_results": 100, "include_retweets": False}}

def _git_source(root: str, vault_name: str) -> dict:
    return {"name": "git-projects", "type": "folder", "enabled": True,
            "url": root, "tool": "muninn.sources.folder:FolderSource",
            "options": {"recursive": True, "max_depth": 3, "include_extensions": [".md"],
                        "include_filenames": ["README.md", "DESIGN.md", "CHANGELOG.md"],
                        "exclude_paths": [vault_name, "node_modules", ".venv", "__pycache__"],
                        "subfolder_as_tag": True, "page_kind": "project"}}

def _docs_source(path: str) -> dict:
    return {"name": "personal-docs", "type": "folder", "enabled": True,
            "url": path, "tool": "muninn.sources.folder:FolderSource",
...
def _claude_source() -> dict:
    return {"name": "claude-history", "type": "claude-history", "enabled": True,
            "url": "~/.claude", "tool": "muninn.sources.claude_history:ClaudeHistorySource",
            "options": {"max_conversations": 20, "min_messages": 3}}

def _youtube_source(vault_name: str) -> dict:
    return {"name": "youtube-transcripts", "type": "youtube", "enabled": True,
            "url": f"./{vault_name}", "tool": "muninn.sources.youtube:YouTubeTranscriptSource",
            "options": {"max_videos": 20, "languages": ["en"],
